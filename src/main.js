...
for (const color of colors) {
    ...
    cardA.setAttribute('data-cy', color);
    ...
    cardB.setAttribute('data-cy', color)
}
...
function onCardClicked(e) {
    ...
    if (preventClick || target === clickedCard) {
        return;
    }
...
if (!clickedCard) {
    ...
    if (clickedCard.getAttribute('data-color') !== target.getAttribute('data-color')) {
        preventClick = true;
        setTimeout(() => {
            clickedCard.className = clickedCard.className.replace('', '').trim() + ' color-hidden';
            target.className = target.className.replace('', '').trim() + ' color-hidden';
            clickedCard = null;
            preventClick = false;
        }, 1000);
    } else {
